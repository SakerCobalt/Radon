...
class Measurement:
    def __init__(self, humidity, temperature, radon_1day):
...
def connect_and_read(device_address):
    ...
    dev.disconnect()
    return humidity,temperature,radon_1day
    ...
    while True:
        try:
            ...
            '''print('{.2f}\t{.2f}\t{radon_1day}'.format(
                humidity,temperature,radon_1day
                ), sep='\t')'''
            sys.stdout.flush()
        if client:
            radon_1day = round(radon_1day/37,1)
            data = ('"'+","+str(temperature)+","+str(humidity)+","+str(radon_1day)+","+'"')
